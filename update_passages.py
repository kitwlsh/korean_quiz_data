...
# 쓰면 ① 토요일 장문이 «금요일 것»으로 만들어지고 ② createdAt·id가 하루 밀린다.
...
#    **다음날 04:40 KST**라 UTC로 보면 «금요일»이 사용자에게는 «토요일»이다.
#    앱은 기기 로컬 날짜(=KST)로 「오늘의 지문」을 고르므로 여기도 KST여야 맞는다.
# 🔴 **토요일로 정했다**(2026-09-15 · 사용자 지시 — 처음엔 일요일이었다). 40~60초짜리는
#    «각오»가 필요한 분량이라 주말 첫날에 두어 **이틀 중 아무 때나** 볼 수 있게 한다.
# 🔴 앱은 이 «장문»을 **글자 수로 판정한다**(별도 필드를 두지 않았다) — JSON 형식을 바꾸지
#    않아야 구버전 앱도 그대로 읽는다. 경계값은 앱의 PassageLength.LONG_CHARS(500자)다.
LONG_WEEKDAY = 5          # 월=0 … 토=5 … 일=6 → **토요일**(KST)
LONG_MIN_CHARS = 800
LONG_MAX_CHARS = 1200
